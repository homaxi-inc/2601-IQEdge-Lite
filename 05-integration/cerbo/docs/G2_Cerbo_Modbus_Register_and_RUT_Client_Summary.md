...
| `DC_System_Power` | **860** | **861** | **16bit INT** | 弱电系统总功耗 W |
| `Time_To_Go` | **846** | **847** | 16bit UINT | 剩余时间（**raw × 100 秒** · 见 §5.1） |

---

## 5. Victron 寄存器定义（Unit ID 100 · `com.victronenergy.system`）

摘自 **CCGX Modbus TCP register list 3.73** — MVP **黄金四件套** + 台架已配扩展。
...
| **846** | `/Dc/Battery/TimeToGo` | uint16 | **0.01** | **s** | **`raw × 100`** 或 `raw / 0.01` | `battery.time_to_go_sec` |
| **850** | `/Dc/Pv/Power` | uint16 | 1 | W | 所有 MPPT 汇总 | `solar.power_w` |
| **860** | `/Dc/System/Power` | int16 | 1 | W | 有符号 | `power_management.dc_system_power_w` |

### 5.1 ⚠️ Time To Go 缩放（易错 · 必读）

Victron GX Modbus-TCP FAQ：**物理值 = raw ÷ scalefactor**。寄存器 **846** 的 scalefactor 为 **0.01**，单位为 **秒**：

```text
time_to_go_sec = raw / 0.01 = raw × 100
```

| raw | 正确（秒 / 小时） | 错误（若当分钟） |
|-----|-------------------|------------------|
| 8640 | **864,000 s = 240 h** | 8640 min = 144 h ❌ |

**台架验证（2026-05-30）**: Modbus raw **8640** ↔ VRM TTG **`240.00 h`**（ST-03 · SmartShunt instance 278）。

RUT Client 只存 **raw**；G2 Data to Server 或云端 ingest 必须 **×100** 再写入 `battery.time_to_go_sec`。**禁止** `× 0.01` 或把 raw 当分钟。
...
| **846** | **847** | **0.01 → ×100 s** | `battery.time_to_go_sec` | §5.1 · **非分钟** |
| **860** | **861** | 1 | `power_management.dc_system_power_w` | 台架已配 |
...
```json
"measures": {
  "battery": { "voltage_v": 26.5, "current_a": -0.4, "soc_pct": 72, "time_to_go_sec": 864000 },
  "solar":   { "power_w": 39 },
